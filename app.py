...
def generate_pdf(form_data: dict) -> bytes:
    ...
    # ── หน้า 1 — กรอก item แรกเท่านั้น (1 item ต่อ 1 หน้า) ──
    def make_overlay(item):
        buf = io.BytesIO()
        ...
        # หัวฟอร์ม
        draw_text(c, supplier,                    195, 753, fs)  # ผู้ส่งมอบ
        draw_text(c, delivery_date,               480, 753, fs)  # วันที่ส่ง

        if item:
            draw_text(c, item.get("material_type",""),  215, 718, fs)  # ชนิดวัตถุดิบ
            draw_text(c, item.get("code",""),           370, 718, fs)  # Code
            draw_text(c, item.get("quantity",""),       490, 718, fs)  # จำนวน
            draw_text(c, item.get("variety",""),        130, 700, fs)  # สายพันธุ์
            draw_text(c, item.get("growing_type",""),   295, 700, fs)  # ลักษณะการปลูก
            draw_text(c, item.get("growing_condition",""), 470, 700, fs)  # ระบบการปลูก
            draw_text(c, item.get("harvest_date",""),   185, 682, fs)  # วันที่เก็บเกี่ยว
            draw_text(c, item.get("harvest_time",""),   435, 682, fs)  # เวลาเก็บเกี่ยว
            draw_text(c, item.get("clean_date",""),     185, 664, fs)  # วันที่ล้าง
            draw_text(c, item.get("clean_time",""),     435, 664, fs)  # เวลาล้าง
            draw_text(c, item.get("grower_name",""),    120, 646, fs)  # ชื่อผู้ปลูก
            draw_text(c, item.get("gap_number",""),     120, 628, fs)  # เลขที่ GAP
            draw_text(c, item.get("farm_code",""),      370, 628, fs)  # รหัสไร่
            # ที่อยู่รวม
            moo_val = f"หมู่ {item.get('moo','')}" if item.get("moo","") else ""
            ...
            draw_text(c, addr, 90, 610, fs, max_width=460)  # ที่อยู่

        # ลงชื่อ/วันที่ (มุมล่างขวา)
        draw_text(c, signer,        420, 108, fs)
        draw_text(c, delivery_date, 420, 93,  fs)

        c.save()
        buf.seek(0)
        return buf

    # สร้างหน้า 1 ต่อ 1 item (หรืออย่างน้อย 1 หน้า)
    items = items if items else [{}]
    for item in items:
        overlay_reader = PdfReader(make_overlay(item))
        page = copy.copy(reader.pages[0])
        page.merge_page(overlay_reader.pages[0])
        writer.add_page(page)

    # หน้าที่เหลือ (หน้า 2, 3 ของ template) ใส่ต่อท้าย
...
<style>
    ...
    /* ซ่อนข้อความ Press Enter to apply */
...
# ─── Thai Data with Zipcode from Excel ──────────────────────────────────────────
@st.cache_data
def load_geo_data():
    ...
    try:
        # รองรับทั้งไฟล์ .xlsx และ .csv เผื่อกรณีแนบไฟล์เป็น csv
        if os.path.exists(file_path_xlsx):
            df = pd.read_excel(file_path_xlsx, dtype=str)
        elif os.path.exists(file_path_csv):
            df = pd.read_csv(file_path_csv, dtype=str)
        else:
            st.warning("⚠️ ไม่พบไฟล์ thailand_with_zip.xlsx ระบบจะไม่สามารถแสดงตัวเลือกที่อยู่ได้")
            return {}
        
        cols = df.columns.tolist()
        # ค้นหาคอลัมน์อัตโนมัติตาม Keyword
        prov_col = next((c for c in cols if 'จังหวัด' in c or 'province' in c.lower()), cols[0])
        dist_col = next((c for c in cols if 'อำเภอ' in c or 'เขต' in c or 'district' in c.lower()), cols[1])
        sub_col = next((c for c in cols if 'ตำบล' in c or 'แขวง' in c or 'subdistrict' in c.lower() or 'tambon' in c.lower()), cols[2])
        zip_col = next((c for c in cols if 'ไปรษณีย์' in c or 'zip' in c.lower() or 'post' in c.lower()), cols[3])

        for _, row in df.iterrows():
            p = str(row[prov_col]).strip()
            d = str(row[dist_col]).strip()
            s = str(row[sub_col]).strip()
            z = str(row[zip_col]).strip()
            
            if z.endswith('.0'): z = z[:-2]  # เผื่อกรณีอ่านแล้วติด .0

            if p not in geo_dict: geo_dict[p] = {}
            if d not in geo_dict[p]: geo_dict[p][d] = {}
            geo_dict[p][d][s] = z
            
    except Exception as e:
        st.error(f"เกิดข้อผิดพลาดในการโหลดข้อมูลที่อยู่: {e}")
...
def get_subdistricts(country, province, district):
    if country == "ไทย" and GEO_DATA and province in GEO_DATA and district in GEO_DATA[province]:
        return sorted(list(GEO_DATA[province][district].keys()))  # .keys() แทน list โดยตรง
    return []
...
def generate_pdf(form_data: dict) -> bytes:
    ...
    fs = 14  # ปรับขนาดตัวอักษร

    # ── บรรทัด 1: ผู้ส่งมอบ / วันที่ส่ง ──
    draw_text(c, supplier,      220, 728, fs, max_width=230)
    draw_text(c, delivery_date, 465, 728, fs, max_width=100)

    # ── วนรายการแรก (template รองรับ 1 item ต่อหน้า) ──
    item = items[0] if items else {}

    # บรรทัด 2: ชนิดวัตถุดิบ / Code / จำนวน
    ...
    # บรรทัด 3: สายพันธุ์ / ลักษณะการปลูก / ระบบการปลูก
...
with col1:
    supplier = st_keyup("ผู้ส่งมอบ (Supplier) *", key="supplier") or ""
with col2:
...
with col4:
    email = st_keyup("อีเมลของผู้ส่งมอบ (สำหรับรับ PDF) *", key="email") or ""
with col5:
    signer = st_keyup("ลงชื่อผู้กรอก", key="signer") or ""
with col6:
...
for i, _ in enumerate(st.session_state.item_list):
    ...
    with c1:
        mat = st_keyup("ชนิดวัตถุดิบที่ส่งให้ทาง CPRAM *", key=f"mat_{i}") or ""
    with c2:
        code = st_keyup("Code (เช่น 71000277)", key=f"code_{i}") or ""
    with c3:
        qty = st_keyup("จำนวน (KG) *", key=f"qty_{i}") or ""
    ...
    with c8:
        grower = st_keyup("ชื่อผู้ปลูก", key=f"grower_{i}") or ""
    with c9:
        gap = st_keyup("เลขที่ GAP", key=f"gap_{i}") or ""

    c10, c11, c12 = st.columns([2, 2, 2])
    with c10:
        farm_code = st_keyup("รหัสไร่", key=f"fcode_{i}") or ""
    with c11:
        addr_no = st_keyup("ที่อยู่เลขที่", key=f"addr_{i}") or ""
    with c12:
        moo = st_keyup("หมู่ที่", key=f"moo_{i}") or ""
    ...
    # ดึงค่ารหัสไปรษณีย์มาแสดงอัตโนมัติ (จะอัปเดตเมื่อเปลี่ยนตำบล)
    auto_zip = ""
    ...
    with cc5:
        st.session_state[f"postal_display_{i}"] = auto_zip
        st.text_input("รหัสไปรษณีย์", key=f"postal_display_{i}", disabled=True)
        postal = auto_zip
    with cc6:
        variety = st_keyup("สายพันธุ์", key=f"variety_{i}") or ""
    with cc7:
...
with st.expander("⚙️ ตั้งค่า SMTP (สำหรับส่งอีเมล – ไม่บังคับ)"):
    ...
    with s1:
        # ช่องตั้งค่าอีเมลยังคงใช้ st.text_input เพื่อไม่ให้ข้อมูลอัปเดตถี่เกินไปเวลายังพิมพ์ไม่เสร็จ
